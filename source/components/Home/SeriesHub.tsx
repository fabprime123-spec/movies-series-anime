...
import { Tv, CalendarDays, Flame, Trophy, Radio } from "lucide-react-native"

interface Props {
  airingToday: CardItem[]
  popular: CardItem[]
  topRated: CardItem[]
  onTheAir: CardItem[]
}

type TabType = "airing_today" | "popular" | "top_rated" | "on_the_air"

export function SeriesHub({ airingToday, popular, topRated, onTheAir }: Props) {
  const { theme, accentColor } = useTheme()
  const [activeTab, setActiveTab] = useState<TabType>("airing_today")
  ...
  }, [activeTab, airingToday, popular, topRated, onTheAir])

  const tabs = [
    { key: "airing_today" as const, label: "Airing Today", icon: CalendarDays },
    { key: "popular" as const, label: "Popular", icon: Flame },
    { key: "top_rated" as const, label: "Top Rated", icon: Trophy },
    { key: "on_the_air" as const, label: "On The Air", icon: Radio }
  ]
    ...
    <View style={[styles.hubContainer, { backgroundColor: theme.card, borderColor: theme.border }]}>
      <View style={styles.hubHeader}>
        <View style={styles.titleContainer}>
          ...
          <Text style={[styles.hubTitle, { color: theme.foreground }]}>Series Hub</Text>
        </View>
